...
if __name__ == '__main__':
    ...
    LATEST_METHDIFF=2 #An indicator variable for whether the methylation difference for the last added cytosine was negative (0) or positive (1) or not yet defined for a new window (2) or methylation difference =0 (3)

    #Loop over the BS-seq file, make LuxUS input files
    for i in range(0,len(BSseq_data)):
        ...
        if len(numpy.intersect1d(numpy.nonzero(count_data[i,:])[0],numpy.nonzero(design_matrix[:,t_covariate])[0]))>=N_required_samples and len(numpy.intersect1d(numpy.nonzero(count_data[i,:])[0],numpy.nonzero(design_matrix[:,t_covariate]==0)[0]))>=N_required_samples: 
            ...
            METHLEVEL1=numpy.float(numpy.sum(meth_data[i,numpy.nonzero(design_matrix[:,t_covariate])[0]]))/numpy.float(numpy.sum(count_data[i,numpy.nonzero(design_matrix[:,t_covariate])[0]]))
            METHLEVEL0=numpy.float(numpy.sum(meth_data[i,numpy.nonzero(design_matrix[:,t_covariate]==0)[0]]))/numpy.float(numpy.sum(count_data[i,numpy.nonzero(design_matrix[:,t_covariate]==0)[0]]))
            CURRENT_METHDIFF=METHLEVEL1-METHLEVEL0
            
            print("DEBUG: METHLEVEL1=%s"%(METHLEVEL1))
            print("DEBUG: METHLEVEL0=%s"%(METHLEVEL0))
            print("DEBUG: CURRENT_METHDIFF=%s"%(CURRENT_METHDIFF))
            
            if CURRENT_METHDIFF<0:
                LATEST_METHDIFF=0
            else:
                if CURRENT_METHDIFF>0:
                    LATEST_METHDIFF=1
        ...
        #Finding the next possible cytosine for checking whether the methylation state is of the same sign (if this is not the last cytosine in the file)
        if i!=(len(BSseq_data)-1):
            for i_add in range(1,len(BSseq_data)-i):
                if len(numpy.intersect1d(numpy.nonzero(count_data[i+i_add,:])[0],numpy.nonzero(design_matrix[:,t_covariate])[0]))>=N_required_samples and len(numpy.intersect1d(numpy.nonzero(count_data[i+i_add,:])[0],numpy.nonzero(design_matrix[:,t_covariate]==0)[0]))>=N_required_samples: 
                    METHLEVEL1_NEXT=numpy.float(numpy.sum(meth_data[i+i_add,numpy.nonzero(design_matrix[:,t_covariate])[0]]))/numpy.float(numpy.sum(count_data[i+i_add,numpy.nonzero(design_matrix[:,t_covariate])[0]]))
                    METHLEVEL0_NEXT=numpy.float(numpy.sum(meth_data[i+i_add,numpy.nonzero(design_matrix[:,t_covariate]==0)[0]]))/numpy.float(numpy.sum(count_data[i+i_add,numpy.nonzero(design_matrix[:,t_covariate]==0)[0]]))
                    NEXT_METHDIFF=METHLEVEL1_NEXT-METHLEVEL0_NEXT
            
                    if NEXT_METHDIFF<0:
                        NEXT_METHDIFF_INDICATOR=0
                    else:
                        if NEXT_METHDIFF>0:
                            NEXT_METHDIFF_INDICATOR=1
                        else:
                            
                            NEXT_METHDIFF_INDICATOR=3
                            
                    print("DEBUG: NEXT_METHDIFF=%s"%(NEXT_METHDIFF))
            
                    if NEXT_METHDIFF_INDICATOR==LATEST_METHDIFF:
                        print("DEBUG: found the next neighbor from index %s. The methylation difference was of the same sign. NEXT_METHDIFF_INDICATOR=%s LATEST_METHDIFF=%s"%(i+i_add,NEXT_METHDIFF_INDICATOR,LATEST_METHDIFF))
                        
                    else:
                        print("DEBUG: found the next neighbor from index %s. The methylation difference was not of the same sign. NEXT_METHDIFF_INDICATOR=%s LATEST_METHDIFF=%s"%(i+i_add,NEXT_METHDIFF_INDICATOR,LATEST_METHDIFF))
                            
                    break



        if (((i==(len(BSseq_data)-1)) and CYTOSINE_INDEX_WINDOW>=1) or (((BSseq_data[i+1][1]-WINDOW_START_COORD)>window_width)) and CYTOSINE_INDEX_WINDOW>=1) or CYTOSINE_INDEX_WINDOW>=window_N_cytosines or (numpy.char.not_equal(BSseq_data[i+1][0],BSseq_data[i][0]) and CYTOSINE_INDEX_WINDOW>=1) or (((NEXT_METHDIFF_INDICATOR!=LATEST_METHDIFF) and (NEXT_METHDIFF_INDICATOR!=3) and (LATEST_METHDIFF!=3)) and CYTOSINE_INDEX_WINDOW>=1):
