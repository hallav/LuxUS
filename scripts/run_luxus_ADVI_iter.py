...
if __name__ == '__main__':
    ...
    parser.add_argument('-k','--ADVI_iter',action='store',dest='ADVI_iter',type=int,required=False,help="The max. number of iterations for ADVI. If not specified, default value %s is used."%(default_N_ADVI_iter))
    ...
    if options.algorithm==1:
        ...
        if  options.ADVI_iter is None:
            print("Number of max. iterations for ADVI was not specified. Default value %s is used."%(default_N_ADVI_iter))
            N_ADVI_iter=default_N_ADVI_iter
        else:
            N_ADVI_iter=options.ADVI_iter
