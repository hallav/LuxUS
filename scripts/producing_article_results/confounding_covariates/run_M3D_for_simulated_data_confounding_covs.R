#This script is for running the M3D tool for multiple simulated data sets 
...
N_data_files_d0=as.numeric(input_args[6])
N_data_files_d1=as.numeric(input_args[7])
...
range_starts <- rep(0,N_data_files)
range_ends <- rep(0,N_data_files)
range_chromosomes <- rep(0,N_data_files) 
  
#Objects for storing the windows into BSRaw objects
window_starts <- rep(0,N_data_files*N_cytosines)
window_ends <- rep(0,N_data_files*N_cytosines)
window_chromosomes <- rep(0,N_data_files*N_cytosines)

totalReads_all <- matrix(NA,nrow=N_data_files*N_cytosines,ncol=N_replicates*2)
methReads_all <-matrix(NA,nrow=N_data_files*N_cytosines,ncol=N_replicates*2)

ind_i=1

for (d in 0:1){
  if (d==0){
    N_data_files_D=N_data_files_d0
  }else{
    N_data_files_D=N_data_files_d1
  }

  for (i in 1:N_data_files_D){
