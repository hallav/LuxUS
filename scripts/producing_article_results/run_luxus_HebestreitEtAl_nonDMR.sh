...
WINDOW=$(sed ''"$SLURM_ARRAY_TASK_ID"'q;d' "$INPUTFOLDER"/HebestreitEtAl_nonDMR_saved_window_indices.txt)
echo "${WINDOW}"

SCRIPTFOLDER=/path/to/scripts

OUTPUTFOLDER=/path/to/output/folder
OUTPUTFILE=LuxUS_BFs_nonDMR.txt
TIMEFILE=/path/to/output/folder/nonDMR_LuxUS_computation_times.txt
...
SIGMAR2_FILE="$OUTPUTFOLDER"/nonDMR_estimated_sigmaR2.txt
SIGMAC2_FILE="$OUTPUTFOLDER"/nonDMR_estimated_sigmaC2.txt
SIGMAE2_FILE="$OUTPUTFOLDER"/nonDMR_estimated_sigmaE2.txt
