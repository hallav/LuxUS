...
if __name__ == '__main__':
    ...
    parser.add_argument('-x','--N_saved_windows',action='store',dest='N_saved_windows',type=int,required=True,help='Number of windows to be saved. The saved windows will be chosen randomly.')
    ...
    #Choosing the random indices of the windows that will be saved. 
    #The script has been already run without saving the windows to see how many windows there will be in total.
    #DMR: 16099 nonDMR: 50476 (50492 when chromosome limitation is taken into account)
    to_be_saved_inds=numpy.random.choice(14022,size=options.N_saved_windows,replace=False)
    ...
    number_of_cytosines_in_window=numpy.zeros(options.N_saved_windows)
    mean_coverage_in_window=numpy.zeros((options.N_saved_windows,N_replicates))

    COULD_BE_SAVED_WINDOW_INDEX=0
    ACTUALLY_SAVED_WINDOW_INDEX=0

    SAVED_WINDOW_INDS=numpy.zeros(options.N_saved_windows)
    ...
    for i in range(0,len(BSseq_data)):
        ...
        if len(numpy.intersect1d(numpy.nonzero(count_data[i,:])[0],numpy.nonzero(design_matrix[:,t_covariate])[0]))>=N_required_samples and len(numpy.intersect1d(numpy.nonzero(count_data[i,:])[0],numpy.nonzero(design_matrix[:,t_covariate]==0)[0]))>=N_required_samples: 
            control_tot=numpy.sum(count_data[i,design_matrix[:,t_covariate]==0])
            case_tot=numpy.sum(count_data[i,design_matrix[:,t_covariate]==1])
            control_meth=numpy.sum(meth_data[i,design_matrix[:,t_covariate]==0])
            case_meth=numpy.sum(meth_data[i,design_matrix[:,t_covariate]==1])
            case_meth_level=float(case_meth)/float(case_tot)
            control_meth_level=float(control_meth)/float(control_tot)
            
            if (control_meth_level==0 and case_meth_level==0) or (control_meth_level==1 and case_meth_level==1) or (control_meth_level==1 and case_meth_level==0) or (control_meth_level==0 and case_meth_level==1):
                print("For cytosine %s both the case and control groups had methylation state of 0 or 1."%(i))
            else:
        ...
        if (((i==(len(BSseq_data)-1)) and CYTOSINE_INDEX_WINDOW>=1) or (((BSseq_data[i+1][1]-WINDOW_START_COORD)>window_width)) and CYTOSINE_INDEX_WINDOW>=1) or CYTOSINE_INDEX_WINDOW>=window_N_cytosines or (numpy.char.not_equal(BSseq_data[i+1][0],BSseq_data[i][0]) and CYTOSINE_INDEX_WINDOW>=1):
            ...
            if pvalue<=options.req_pval:    
                ...
                if int(CYTOSINE_INDEX_WINDOW) > 1:

                    if numpy.in1d(COULD_BE_SAVED_WINDOW_INDEX,to_be_saved_inds)[0]:  
                        ...
                        number_of_cytosines_in_window[ACTUALLY_SAVED_WINDOW_INDEX]=CYTOSINE_INDEX_WINDOW
                        mean_coverage_in_window[ACTUALLY_SAVED_WINDOW_INDEX,:]=numpy.mean(counts_window[0:CYTOSINE_INDEX_WINDOW,:],axis=0)
                        SAVED_WINDOW_INDS[ACTUALLY_SAVED_WINDOW_INDEX]=INCLUDED_IN_ANALYSIS_WINDOW_SAVED
                        ACTUALLY_SAVED_WINDOW_INDEX = ACTUALLY_SAVED_WINDOW_INDEX+1

                    COULD_BE_SAVED_WINDOW_INDEX = COULD_BE_SAVED_WINDOW_INDEX+1    
    ...
    numpy.savetxt("%s/%s_in_analysis_indicator.txt"%(options.output_folder,outputid),INCLUDED_IN_ANALYSIS,fmt='%10.0f')
    if  options.cytNFile is None:
        ...
        if INCLUDED_IN_ANALYSIS_WINDOW_SAVED==1:
            ...
            numpy.savetxt("%s"%(options.cytNFile),number_of_cytosines_in_window,fmt='%10.0f')  
    if  options.meanCovFile is None:
        ...
        if INCLUDED_IN_ANALYSIS_WINDOW_SAVED==1:
            ...
            numpy.savetxt("%s"%(options.meanCovFile),mean_coverage_in_window,fmt='%10.5f')

    numpy.savetxt("%s/%s_saved_window_inds.txt"%(options.output_folder,outputid),SAVED_WINDOW_INDS,fmt='%10.0f')
